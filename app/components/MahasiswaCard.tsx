...
export const MahasiswaCard = ({ mhs, index }: MahasiswaCardProps) => {
    ...
    // useRouter tidak lagi dibutuhkan karena <Link> menangani navigasi
    
    return (
        // 1. Seluruh kartu sekarang dibungkus oleh komponen Link
        <Link
            href={`/mahasiswa/detail/${encodeURIComponent(mhs.id)}`}
            onClick={() => setIsLoading(true)}
            className="group block" // 'block' agar Link mengisi seluruh area
        >
            <div 
            ...
            >
                <div className="flex justify-between items-start">
                    ...
                    {/* 2. Tombol panah sekarang hanya menjadi indikator visual di dalam Link */}
                    <div
