...
export const PtSearchableSelect = ({ value, onChange, placeholder = "Ketik untuk mencari perguruan tinggi..." }: PtSearchableSelectProps) => {
    ...
    const fetchPt = useCallback(async (query: string) => {
        ...
        try {
            ...
            console.error("Gagal mengambil data perguruan tinggi:", error);
            setOptions([]);
        } finally {
            setLoading(false);
        }
    ...
    useEffect(() => {
        fetchPt(debouncedSearchTerm);
    }, [debouncedSearchTerm, fetchPt]);

    const handleSelect = (option: PerguruanTinggi) => {
        onChange(option);
        setSearchTerm("");
        setOptions([]);
        setIsOpen(false);
    ...
    const handleBlur = () => {
        setTimeout(() => {
            if (!document.activeElement || !document.activeElement.closest('.pt-select-dropdown-portal')) {
                setIsOpen(false);
                setSearchTerm("");
            }
        }, 150);
                ...
                >
                    <div className="flex items-center gap-3 flex-grow min-w-0">
                        ...
                        <div className="flex-grow min-w-0">
                            {value ? (
                                <p className="text-sm font-semibold text-gray-800 truncate">{value.nama}</p>
                            ) : (
                                    ...
                                    {options.length > 0 ? (
                                        options.map(option => (
                                            <li key={option.id} onMouseDown={() => handleSelect(option)} className="p-4 cursor-pointer hover:bg-blue-50 text-gray-800 border-b border-gray-100 last:border-b-0 transition-colors">
                                                <p className="font-semibold text-sm flex items-center gap-2"><University size={14}/> {option.nama}</p>
                                                <p className="text-xs text-gray-500 flex items-center gap-2 mt-1 pl-6">Kode: {option.kode}</p>
                                            </li>
                                        ))
                                    ) : (
                                        <div className="flex flex-col items-center justify-center text-center p-8 text-sm text-gray-500">
                                            ...
                                            {!loading && debouncedSearchTerm.length >= 3 && <p>Perguruan Tinggi <strong className="text-gray-600">"{debouncedSearchTerm}"</strong> tidak ditemukan.</p>}
