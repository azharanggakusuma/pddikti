...
export const PtSearchableSelect = ({
  value,
  onChange,
  placeholder = 'Ketik untuk mencari perguruan tinggi...',
}: PtSearchableSelectProps) => {
  ...
  const fetchPt = useCallback(async (query: string) => {
    ...
    try {
      ...
      setOptions(Array.isArray(data) ? (data as PerguruanTinggi[]) : []);
    } catch (error) {
                  ...
                  {options.length > 0 ? (
                    options.map((option) => (
                      ...
                      >
                        <p className="font-semibold text-sm flex items-center gap-2">
                          <University size={14} /> {option.nama}
                        </p>
                    ...
                    <div className="flex flex-col items-center justify-center text-center p-8 text-sm text-gray-500">
                      ...
                      {!loading && debouncedSearchTerm.length < 3 && (
                        <p>
                          Ketik minimal <strong className="text-gray-600">3 huruf</strong> untuk memulai.
                        </p>
                      )}
                      {!loading && debouncedSearchTerm.length >= 3 && (
                        <p>
