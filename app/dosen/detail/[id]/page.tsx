...
const DetailSkeleton = () => (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8 antialiased">
        <main className="max-w-3xl mx-auto">
          <Breadcrumbs items={[{ label: 'Dosen', href: '/dosen' }, {label: 'Detail'}]} />
...
export default function DosenDetailPage() {
  const { data: dosen, loading, error } = useDetailPage<DosenDetail>('dosen');

  const breadcrumbItems = [
    { label: 'Dosen', href: '/dosen' },
    { label: dosen ? dosen.nama_dosen : 'Detail' },
  ];
      ...
      <main className="max-w-3xl mx-auto">
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-xl shadow-gray-200/50 overflow-hidden border border-gray-200">
          ...
          <div className="p-6 sm:p-8 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <InfoItem label="Pendidikan Tertinggi" value={dosen.pendidikan_tertinggi} icon={<GraduationCap size={20} />} />
            <InfoItem label="Status Ikatan Kerja" value={dosen.status_ikatan_kerja} icon={<Briefcase size={20} />} />
