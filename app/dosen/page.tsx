...
export default function DosenPage() {
    ...
    const fetchResults = async () => {
      ...
      try {
        const response = await fetch(
          `/api/dosen?q=${encodeURIComponent(query)}`
        );
  ...
  const processedResults = useMemo(() => {
    return allResults
      ...
      .filter(
        (dosen) => filterProdi === "Semua" || dosen.nama_prodi === filterProdi
      )
      .sort((a, b) => {
  ...
  const uniquePT = useMemo(
    () => ["Semua", ...new Set(allResults.map((dosen) => dosen.nama_pt))],
    [allResults]
  );
  const uniqueProdi = useMemo(
    () => ["Semua", ...new Set(allResults.map((dosen) => dosen.nama_prodi))],
    [allResults]
  );
