...
function LocationSection({
  ...
  // Parser koordinat aman (koma -> titik)
  ...
  }, [mapVisible, coordsValid, lat, lng, zoom, mapType]);
  ...
  return (
    <div className="mt-12">
      ...
      <div className="relative rounded-2xl border border-gray-200 bg-white/70 backdrop-blur-sm shadow-xl overflow-hidden">
        ...
        <div
          ref={mapDivRef}
          className="relative z-0 aspect-video w-full bg-gradient-to-b from-gray-50 to-white"
        >
          {!mapLoaded && (
          ...
          {/* overlay bawah kecil (info koordinat) */}
          <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[500] p-4">
            <div className="pointer-events-auto mx-auto max-w-fit rounded-xl border border-gray-200 bg-white/90 shadow backdrop-blur px-3 py-1.5 text-xs text-gray-600 flex items-center gap-2">
              <MapPin size={14} className="text-blue-600" />
              {coordsValid ? `${lat.toFixed(6)}, ${lng.toFixed(6)}` : 'Koordinat tidak valid'}
            </div>
          </div>
        </div>
        ...
        <div className="absolute z-[10000] top-4 left-4 flex items-center gap-2">
          <div className="inline-flex p-1 rounded-xl border border-gray-200 bg-white/90 shadow pointer-events-auto">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium ${mapType === 'osm' ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
            >
              Map
            </button>
            <button
              onClick={() => setMapType('sat')}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium ${mapType === 'sat' ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
            >
              Satellite
            </button>
          </div>
          ...
          <div className="hidden md:inline-flex items-center gap-1 p-1 rounded-xl border border-gray-200 bg-white/90 shadow pointer-events-auto">
            ...
            <div className="px-2 text-sm text-gray-600 tabular-nums">{zoom}</div>
            <button
