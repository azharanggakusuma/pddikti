...
export default function ProdiPage() {
    ...
    const fetchResults = async () => {
      ...
      try {
        const response = await fetch(
          `/api/prodi?q=${encodeURIComponent(query)}`
        );
  ...
  const processedResults = useMemo(() => {
    return allResults
      ...
      .filter(
        (prodi) => filterJenjang === "Semua" || prodi.jenjang === filterJenjang
      )
      .sort((a, b) => {
  ...
  const uniquePT = useMemo(
    () => ["Semua", ...new Set(allResults.map((prodi) => prodi.pt))],
    [allResults]
  );
  const uniqueJenjang = useMemo(
    () => ["Semua", ...new Set(allResults.map((prodi) => prodi.jenjang))],
    [allResults]
  );
        ...
        <div className="grid grid-cols-1 gap-5">
          ...
          {!loading &&
            paginatedResults.map((prodi, index) => (
              <ProdiCard key={prodi.id} prodi={prodi} index={index} />
            ))}
        </div>
