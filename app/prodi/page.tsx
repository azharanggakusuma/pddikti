...
import { FileX, ArrowUp, ChevronLeft, ChevronRight, Search, History, Loader2, X, SlidersHorizontal, BookOpen } from 'lucide-react';
...
export default function ProdiPage() {
    ...
    // State hooks
    const [allResults, setAllResults] = useState<ProgramStudi[]>([]);
    ...
    const processedResults = useMemo(() => {
        return allResults
            .filter(prodi => (filterPT === 'Semua' || prodi.pt === filterPT))
            .filter(prodi => (filterJenjang === 'Semua' || prodi.jenjang === filterJenjang))
            .sort((a, b) => {
                if (sortBy === 'nama-asc') return a.nama.localeCompare(b.nama);
                if (sortBy === 'nama-desc') return b.nama.localeCompare(a.nama);
                return 0;
            });
    }, [allResults, filterPT, filterJenjang, sortBy]);
        ...
        <div className="min-h-screen p-4 sm:p-8 flex flex-col items-center antialiased bg-gray-50 text-gray-800">
            <main className="w-full max-w-4xl mx-auto">
                        ...
                        {showFilters && (
                            <div className="bg-white p-4 rounded-xl border border-gray-200 mb-4" style={{ animation: 'fadeInUp 0.3s ease-out' }}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="text-xs font-semibold text-gray-500">Jenjang</label>
                                        <select value={filterJenjang} onChange={e => { setFilterJenjang(e.target.value); setCurrentPage(1); }} className="mt-1 w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-white">
                                            {uniqueJenjang.map(jenjang => <option key={jenjang} value={jenjang}>{jenjang}</option>)}
                                        </select>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-1 gap-5">
                    ...
                    {!loading && paginatedResults.map((prodi, index) => <ProdiCard key={prodi.id} prodi={prodi} index={index} />)}
                </div>
