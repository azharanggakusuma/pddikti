...
export default function MahasiswaPage() {
  ...
  const generateSuggestion = (text: string) => {
    if (text.toLowerCase().includes("muhamad"))
      return text.replace(/muhamad/i, "Muhammad");
    if (text.toLowerCase().includes("univ"))
      return text.replace(/univ/i, "Universitas");
    return null;
  };
    ...
    const fetchResults = async () => {
      ...
      try {
        const response = await fetch(
          `/api/mahasiswa?q=${encodeURIComponent(query)}`
        );
  ...
  const processedResults = useMemo(() => {
    return allResults
      ...
      .filter(
        (mhs) => filterProdi === "Semua" || mhs.nama_prodi === filterProdi
      )
      .sort((a, b) => {
  ...
  const uniquePT = useMemo(
    () => ["Semua", ...new Set(allResults.map((mhs) => mhs.nama_pt))],
    [allResults]
  );
  const uniqueProdi = useMemo(
    () => ["Semua", ...new Set(allResults.map((mhs) => mhs.nama_prodi))],
    [allResults]
  );
          ...
          {!loading && !error && !query && (
            <div className="text-center text-gray-500 border-2 border-dashed border-gray-300 p-10 sm:p-16 rounded-xl flex flex-col items-center justify-center">
              ...
              <p className="text-sm sm:text-base mt-1">
                Gunakan kotak pencarian di atas untuk menemukan data mahasiswa.
              </p>
            </div>
          )}
            ...
            <div className="text-center text-gray-500 border-2 border-dashed border-gray-300 p-10 sm:p-16 rounded-xl flex flex-col items-center justify-center">
              ...
              {suggestion && (
                <p className="text-sm sm:text-base mt-4">
                  Mungkin maksud Anda:{" "}
                  <button
                    onClick={() => handleNewSearch(undefined, suggestion)}
